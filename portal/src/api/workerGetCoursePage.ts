...
export async function workerGetCoursePage(
  ...
  // const sampleCourseContent = CoursePageSchema.parse(sampleJson);
  // console.log("Sample Course page content:", sampleCourseContent);
  // return sampleCourseContent;

  try {
    // ("input:", input);
    input = CoursePageInputSchema.parse(input);
    // ("Input Parsed:", input);
  } catch (error) {
  ...
  const URL = WORKER_BASE_URL + "/getCoursePage";
  ...
  // ("Course page response:", courseResponse);
  ...
  const courseContent = CoursePageSchema.parse(courseJson);
  // ("Course page content:", courseContent);
